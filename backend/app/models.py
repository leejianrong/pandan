...
class Board(Base):
    ...
    # The workspace this board belongs to (M9 V65, KAN-1054; ADR 0021). Nullable, ON
    # DELETE SET NULL — mirrors ``owner_id``'s own pattern exactly: deleting a workspace
    # unclaims its boards rather than destroying them. ``owner_id`` stays untouched
    # and still required — a board keeps exactly one human owner; ``workspace_id`` is an
    # additional grouping/sharing pointer layered on top, not a replacement for
    # ownership. A board belongs to at most one workspace (no join table, ADR 0021
    ...
    workspace_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("workspace.id", ondelete="SET NULL"), nullable=True
    )
...
class Workspace(Base):
    ...
    grouping of members and, via ``Board.workspace_id``, of boards.

    Deliberately **no ``owner_id``**: unlike a board, a workspace isn't administered by
    one person by default — it's administered by whichever of its members hold the
    ``owner`` role (see :class:`WorkspaceMember`). Deliberately **no ``key`` either** — a
    workspace has no ticket-ref namespace of its own; that stays exactly where ADR 0020
    put it (per-user, not per-workspace — see ADR 0021 §Consequences).
    """
...
class WorkspaceMember(Base):
    """A user's membership of a workspace with a role (M9 V65, KAN-1054; ADR 0021).
    ...
    ``UNIQUE(workspace_id, user_id)`` keeps a user's membership of a workspace singular. Both
    FKs ``ON DELETE CASCADE`` — deleting a workspace or a user removes the membership row
    (consistent with the app's hard-delete model).

    Workspace membership grants a workspace **default** board access once V68 lands (an
    ...
    workspace_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("workspace.id", ondelete="CASCADE"), nullable=False, index=True
    )
    # The member user (a UUID). CASCADE: deleting a user removes their memberships.
    user_id: Mapped[uuid.UUID] = mapped_column(
        GUID, ForeignKey("user.id", ondelete="CASCADE"), nullable=False
