...
**Workspace linking (M9 V67, KAN-1056; ADR 0021).** ``workspace_id`` on create/update is
optional and 403s if the caller isn't a member of the named workspace (any role) —
:func:`_reject_non_member_workspace`. Omitted on create → ``NULL`` (a personal board,
...
def _reject_non_member_workspace(db: Session, principal: User, workspace_id: int | None) -> None:
    """403 if ``workspace_id`` is set and the principal isn't a member of it — any role
    (M9 V67, KAN-1056; ADR 0021 §New surface). Uniform for an unknown workspace too (no
    row → no membership row → the same 403), so a create/update can't be used to
    probe which workspace ids exist — "you don't get to point a create at something you
...
def create_board(
    ...
    _reject_non_member_workspace(db, principal, payload.workspace_id)
    ...
    for attempt in range(_KEY_ALLOCATION_ATTEMPTS):
        ...
        board = Board(
            name=payload.name, key=key, owner_id=principal.id, workspace_id=payload.workspace_id
        )
        db.add(board)
        try:
            db.commit()
...
def update_board(
    ...
    if "workspace_id" in data:
        ...
        _reject_non_member_workspace(db, principal, data["workspace_id"])
