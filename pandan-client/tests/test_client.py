...
def test_create_board_sends_workspace_id_when_given():
    import json

    handler, seen = capture(httpx.Response(201, json={"id": 2, "name": "New", "workspace_id": 9}))
    make_client(handler).create_board("New", workspace_id=9)
    assert json.loads(seen["content"]) == {"name": "New", "workspace_id": 9}


def test_update_board_sends_workspace_id_when_given():
    import json

    handler, seen = capture(httpx.Response(200, json={"id": 2, "workspace_id": 9}))
    make_client(handler).update_board(2, workspace_id=9)
    assert json.loads(seen["content"]) == {"workspace_id": 9}


def test_update_board_omits_workspace_id_when_not_given():
    import json
...
# --- workspaces (M9 V65-V68; ADR 0021) -------------------------------------------


def test_list_workspaces_hits_workspaces_and_wraps_result():
    handler, seen = capture(httpx.Response(200, json=[{"id": 1, "name": "Platform"}]))
    out = make_client(handler).list_workspaces()
    assert seen["method"] == "GET"
    assert seen["path"] == "/api/v1/workspaces"
    assert out == {"workspaces": [{"id": 1, "name": "Platform"}]}
...
def test_create_workspace_posts_name():
    ...
    out = make_client(handler).create_workspace("Platform")
    assert seen["method"] == "POST"
    assert seen["path"] == "/api/v1/workspaces"
...
def test_delete_workspace_sends_delete_and_returns_ack_without_parsing_body():
    handler, seen = capture(httpx.Response(204))
    out = make_client(handler).delete_workspace(4)
    assert seen["method"] == "DELETE"
    assert seen["path"] == "/api/v1/workspaces/4"
    assert out == {"deleted": 4}


# --- workspace membership (M9 V66, KAN-1055) -------------------------------------


def test_list_workspace_members_hits_the_members_path_and_wraps_result():
    handler, seen = capture(httpx.Response(200, json=[{"id": 1, "role": "owner"}]))
    out = make_client(handler).list_workspace_members(4)
    assert seen["method"] == "GET"
    assert seen["path"] == "/api/v1/workspaces/4/members"
    assert out == {"members": [{"id": 1, "role": "owner"}]}


def test_add_workspace_member_by_email_defaults_role_viewer():
    import json

    handler, seen = capture(httpx.Response(201, json={"id": 5, "role": "viewer"}))
    make_client(handler).add_workspace_member(4, email="bob@example.com")
    assert seen["method"] == "POST"
    assert seen["path"] == "/api/v1/workspaces/4/members"
    assert json.loads(seen["content"]) == {"email": "bob@example.com", "role": "viewer"}


def test_add_workspace_member_by_user_id_with_role():
    import json

    handler, seen = capture(httpx.Response(201, json={"id": 5, "role": "editor"}))
    make_client(handler).add_workspace_member(4, user_id="uuid-1", role="editor")
    assert json.loads(seen["content"]) == {"user_id": "uuid-1", "role": "editor"}


def test_update_workspace_member_patches_role():
    import json

    handler, seen = capture(httpx.Response(200, json={"id": 5, "role": "editor"}))
    out = make_client(handler).update_workspace_member(4, 5, role="editor")
    assert seen["method"] == "PATCH"
    assert seen["path"] == "/api/v1/workspaces/4/members/5"
    assert json.loads(seen["content"]) == {"role": "editor"}
    assert out == {"id": 5, "role": "editor"}


def test_remove_workspace_member_sends_delete_and_returns_ack():
    handler, seen = capture(httpx.Response(204))
    out = make_client(handler).remove_workspace_member(4, 5)
    assert seen["method"] == "DELETE"
    assert seen["path"] == "/api/v1/workspaces/4/members/5"
