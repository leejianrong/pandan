...
ln -sf ~/.local/bin/pandan ~/.local/bin/pdn       # the `pdn` alias — see note below

# or, if you have Python + uv:
uv tool install "git+https://github.com/leejianrong/pandan.git#subdirectory=pandan-cli"
```

> **The `pdn` alias only comes free on the `uv tool install` path.** `[project.scripts]` declares both
> `pandan` and `pdn`, but the release is a PyInstaller `--onefile` build that produces exactly **one**
> executable — so a binary install gives you `pandan` and no `pdn` unless you symlink it, as above
> (KAN-442). The same trick is worth applying to the retired name if you have the muscle memory:
> `ln -sf ~/.local/bin/pandan ~/.local/bin/kan` — which also guarantees the old name isn't a stale
> pre-rebrand build sitting on your `PATH`.
